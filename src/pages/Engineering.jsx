...
const Engineering = () => {
  ...
  // UI states for progress and preview
  ...
  useEffect(() => {
    ...
    if (urlChatId) {
      setChatId(urlChatId);
      if (sessionStorage.getItem('chatId-engineering') !== urlChatId) {
        sessionStorage.setItem('chatId-engineering', urlChatId);
      }
      if (localStorage.getItem('chatId-engineering') !== urlChatId) {
        localStorage.setItem('chatId-engineering', urlChatId);
      }
    } else {
      ...
      async function findChat() {
        ...
        if (!snapshot.empty) {
          ...
          setChatId(chatDocId);
          sessionStorage.setItem('chatId-engineering', chatDocId);
          localStorage.setItem('chatId-engineering', chatDocId);
  ...
  const handleSend = async (e) => {
    ...
    if (!currentChatId) {
      ...
      sessionStorage.setItem('chatId-engineering', currentChatId);
      localStorage.setItem('chatId-engineering', currentChatId);
  ...
  const sendOrderDetails = async (e) => {
    ...
    const requestsRef = collection(db, 'requests-engineering');
        ...
        {chatId && (
          <div className="alert alert-info mt-3" style={{ fontSize: 15, direction: 'ltr', position: 'relative', zIndex: 2 }}>
            ...
            <div style={{ wordBreak: 'break-all', margin: '8px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <a
                href={`${window.location.origin}/engineering?chatId=${chatId}`}
                target="_blank"
                rel="noopener noreferrer"
                style={{ fontWeight: 'bold', color: '#0d6efd', textDecoration: 'underline' }}
              >
                {`${window.location.origin}/engineering?chatId=${chatId}`}
              </a>
              <button
                className="btn btn-sm btn-outline-secondary"
                style={{ marginLeft: 8, fontSize: 13 }}
                onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/engineering?chatId=${chatId}`);
                }}
