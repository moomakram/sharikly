...
const Engineering = () => {
  ...
  useEffect(() => {
    const storedChatId = sessionStorage.getItem('chatId-engineering');
    if (storedChatId) {
      setChatId(storedChatId);
    } else {
      const newId = uuidv4();
      sessionStorage.setItem('chatId-engineering', newId);
      setChatId(newId);
  ...
  const sendOrderDetails = async (e) => {
    ...
    // حفظ الطلب في قاعدة بيانات الطلبات
    await addDoc(collection(db, 'requests-engineering'), {
      chatId,
      details,
      timestamp: serverTimestamp(),
    });

    // إرسال الطلب إلى الشات مباشرة
    await addDoc(collection(db, 'chats', chatId, 'messages'), {
  ...
  return (
    <div className="container py-4">
      <h2 className="text-center mb-4">شات قسم التصميم الهندسي</h2>

      <div className="row d-flex justify-content-center gap-5">
        {/* Chat Section */}
        <div className="card shadow-sm col-md-5" style={{ marginBottom: '30px' }}>
          <div className="card-header bg-primary text-white text-center">
            <h5 className="mb-0">محادثة قسم التصميم الهندسي</h5>
          </div>
        ...
        <div className="card shadow-sm col-md-5" style={{ marginBottom: '30px' }}>
          <div className="card-header bg-primary text-white text-center">
            <h5 className="mb-0">طلب خدمة تصميم هندسي</h5>
          </div>
