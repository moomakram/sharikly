...
const Programming = () => {
  ...
  useEffect(() => {
    ...
    if (urlChatId) {
      setChatId(urlChatId);
      if (sessionStorage.getItem('chatId-programming') !== urlChatId) {
        sessionStorage.setItem('chatId-programming', urlChatId);
      }
      if (localStorage.getItem('chatId-programming') !== urlChatId) {
        localStorage.setItem('chatId-programming', urlChatId);
      }
    } else {
      ...
      async function findChat() {
        ...
        if (!snapshot.empty) {
          ...
          setChatId(chatDocId);
          sessionStorage.setItem('chatId-programming', chatDocId);
          localStorage.setItem('chatId-programming', chatDocId);
  ...
  const handleSend = async (e) => {
    ...
    if (!currentChatId) {
      ...
      sessionStorage.setItem('chatId-programming', currentChatId);
      localStorage.setItem('chatId-programming', currentChatId);
  ...
  const sendOrderDetails = async (e) => {
    ...
    const requestsRef = collection(db, 'requests-programming');
        ...
        {chatId && (
          <div className="alert alert-info mt-3" style={{ fontSize: 15, direction: 'ltr', position: 'relative', zIndex: 2 }}>
            ...
            <div style={{ wordBreak: 'break-all', margin: '8px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <a
                href={`${window.location.origin}/programming?chatId=${chatId}`}
                target="_blank"
                rel="noopener noreferrer"
                style={{ fontWeight: 'bold', color: '#0d6efd', textDecoration: 'underline' }}
              >
                {`${window.location.origin}/programming?chatId=${chatId}`}
              </a>
              <button
                className="btn btn-sm btn-outline-secondary"
                style={{ marginLeft: 8, fontSize: 13 }}
                onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/programming?chatId=${chatId}`);
                }}
