...
const Programming = () => {
  ...
  useEffect(() => {
    const storedChatId = sessionStorage.getItem('chatId-programming');
    if (storedChatId) {
      setChatId(storedChatId);
    } else {
      const newId = uuidv4();
      sessionStorage.setItem('chatId-programming', newId);
      setChatId(newId);
  ...
  const sendOrderDetails = async (e) => {
    ...
    await addDoc(collection(db, 'requests-programming'), {
      chatId,
      details,
      timestamp: serverTimestamp(),
    });
  ...
  return (
    <div className="container py-4">
      <h2 className="text-center mb-4">شات قسم البرمجة</h2>

      <div className="row d-flex justify-content-center gap-5">
        {/* Chat Section */}
        <div className="card shadow-sm col-md-5" style={{ marginBottom: '30px' }}>
          <div className="card-header bg-primary text-white text-center">
            <h5 className="mb-0">محادثة قسم البرمجة</h5>
          </div>
        ...
        <div className="card shadow-sm col-md-5" style={{ marginBottom: '30px' }}>
          <div className="card-header bg-primary text-white text-center">
            <h5 className="mb-0">طلب خدمة برمجية</h5>
          </div>
