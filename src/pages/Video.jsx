 ...
 * - يظهر أثناء التمرير ويختفي بعد ثانيتين من توقف التمرير
 * - يحدد اتجاه السهم حسب اتجاه التمرير (⬆ أو ⬇)
 * - ينقلك لأول أو آخر رسالة بسلاسة عند الضغط
 * - لون الزر يتغير تلقائياً حسب الخلفية والوضع الليلي
 */
const ScrollButton = ({ containerRef, bottomRef, darkMode, small, fullColor }) => {
  ...
  // ألوان كاملة حسب الاتجاه والخلفية
  let bgColor = "#0d6efd";
  let color = "#fff";
  if (fullColor) {
    if (direction === "up") {
...
const Video = () => {
  ...
  useEffect(() => {
    ...
    if (urlChatId) {
      setChatId(urlChatId);
      if (sessionStorage.getItem('chatId-video') !== urlChatId) {
        sessionStorage.setItem('chatId-video', urlChatId);
      }
      if (localStorage.getItem('chatId-video') !== urlChatId) {
        localStorage.setItem('chatId-video', urlChatId);
      }
    } else {
      ...
      async function findChat() {
        ...
        if (!snapshot.empty) {
          ...
          setChatId(chatDocId);
          sessionStorage.setItem('chatId-video', chatDocId);
          localStorage.setItem('chatId-video', chatDocId);
  ...
  const handleSend = async (e) => {
    ...
    if (!currentChatId) {
      ...
      sessionStorage.setItem('chatId-video', currentChatId);
      localStorage.setItem('chatId-video', currentChatId);
  ...
  const sendOrderDetails = async (e) => {
    ...
    const requestsRef = collection(db, 'requests-video');
        ...
        {chatId && (
          <div className="alert alert-info mt-3" style={{ fontSize: 15, direction: 'ltr', position: 'relative', zIndex: 2 }}>
            ...
            <div style={{ wordBreak: 'break-all', margin: '8px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <a
                href={`${window.location.origin}/video?chatId=${chatId}`}
                target="_blank"
                rel="noopener noreferrer"
                style={{ fontWeight: 'bold', color: '#0d6efd', textDecoration: 'underline' }}
              >
                {`${window.location.origin}/video?chatId=${chatId}`}
              </a>
              <button
                className="btn btn-sm btn-outline-secondary"
                style={{ marginLeft: 8, fontSize: 13 }}
                onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/video?chatId=${chatId}`);
                }}
          ...
          >
            <div className="card-header bg-primary text-white text-center d-flex justify-content-between align-items-center" style={{ position: 'relative' }}>
              ...
              <div style={{
                position: "absolute",
                right: 18,
                bottom: -60, // نزّل الزر أكثر ليكون أسفل زر الإبلاغ بوضوح أكبر
                zIndex: 20
              }}>
                <ScrollButton
                  containerRef={chatMessagesBoxRef}
