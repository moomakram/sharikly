...
import { v4 as uuidv4 } from 'uuid'; // إذا لم يكن لديك userId من auth
import { useTheme } from '../context/ThemeContext';
import { useTranslation } from 'react-i18next';
import { useLocation, useNavigate } from 'react-router-dom';
import { Modal, Button } from 'react-bootstrap';
import EmojiPicker from 'emoji-picker-react'; // أضف هذا السطر
...
const UIUX = () => {
  ...
  const [showEmojiPicker, setShowEmojiPicker] = useState(false); // حالة إظهار الإيموجي

  // أضف state للنسبة والمعاينة للملفات
  ...
  useEffect(() => {
    ...
    async function findChat() {
      ...
      if (!snapshot.empty) {
        ...
        setChatId(chatDocId);
        sessionStorage.setItem('chatId-uiux', chatDocId);
        localStorage.setItem('chatId-uiux', chatDocId);
  ...
  useEffect(() => {
    ...
    if (lastMsg.sender === 'client') {
      ...
      getDoc(chatDocRef).then((docSnap) => {
        if (docSnap.exists() && docSnap.data().autoReplySent) return;
        setTimeout(async () => {
          let autoReply = "";
  ...
  const handleChatFileSelect = (e) => {
    ...
    if (file) {
      ...
      // إعادة تعيين قيمة input حتى يمكن رفع نفس الملف بعد الإلغاء
  ...
  const handleSend = async (e) => {
    ...
    if (!currentChatId) {
      ...
      sessionStorage.setItem('chatId-uiux', currentChatId);
      localStorage.setItem('chatId-uiux', currentChatId);
    ...
    // تحديث بيانات الشات في وثيقة الشات
  ...
  const sendOrderDetails = async (e) => {
    ...
    const requestsRef = collection(db, 'requests-uiux');
        ...
        {chatId && (
          <div className="alert alert-info mt-3" style={{ fontSize: 15, direction: 'ltr', position: 'relative', zIndex: 2 }}>
            ...
            <div style={{ wordBreak: 'break-all', margin: '8px 0', display: 'flex', alignItems: 'center', gap: 8 }}>
              <a
                href={`${window.location.origin}/uiux?chatId=${chatId}`}
                target="_blank"
                rel="noopener noreferrer"
                style={{ fontWeight: 'bold', color: '#0d6efd', textDecoration: 'underline' }}
              >
                {`${window.location.origin}/uiux?chatId=${chatId}`}
              </a>
              <button
                className="btn btn-sm btn-outline-secondary"
                style={{ marginLeft: 8, fontSize: 13 }}
                onClick={() => {
                  navigator.clipboard.writeText(`${window.location.origin}/uiux?chatId=${chatId}`);
                }}
        ...
        <div className="d-flex flex-wrap justify-content-center gap-4 video-flex-responsive align-items-stretch">
          ...
          >
            ...
            <div
              ...
              style={{ overflowY: 'auto', maxHeight: '300px', position: 'relative' }}
              ref={messagesBoxRef}
            >
