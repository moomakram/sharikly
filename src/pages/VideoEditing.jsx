...
const VideoEditing = () => {
  ...
  useEffect(() => {
    const storedChatId = sessionStorage.getItem('chatId-video');
    if (storedChatId) {
      setChatId(storedChatId);
    } else {
      const newId = uuidv4();
      sessionStorage.setItem('chatId-video', newId);
      setChatId(newId);
  ...
  const sendOrderDetails = async (e) => {
    ...
    await addDoc(collection(db, 'requests-video'), {
      chatId,
      details,
      timestamp: serverTimestamp(),
    });
  ...
  return (
    <div className="container py-4">
      <h2 className="text-center mb-4">شات قسم الفيديو والمونتاج</h2>

      <div className="row d-flex justify-content-center gap-5">
        {/* Chat Section */}
        <div className="card shadow-sm col-md-5" style={{ marginBottom: '30px' }}>
          <div className="card-header bg-primary text-white text-center">
            <h5 className="mb-0">محادثة قسم الفيديو والمونتاج</h5>
          </div>
        ...
        <div className="card shadow-sm col-md-5" style={{ marginBottom: '30px' }}>
          <div className="card-header bg-primary text-white text-center">
            <h5 className="mb-0">طلب خدمة مونتاج فيديو</h5>
          </div>
